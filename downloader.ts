...
// Configuration
const CONFIG = {
  downloadDelay: 500, // ms between downloads to avoid overwhelming the server
  outputDir: './downloads',
  skipExisting: true, // Skip files that already exist
  retryCount: 3, // Number of retry attempts for failed downloads
  retryDelay: 2000, // ms to wait before retrying
  booksJsonPath: './books.json', // Path to books.json
  updateJson: true, // Update books.json after successful downloads
  addPathInfo: true, // Add path and fileName information to chapters
...
function removeDuplicateChapters(classes: ClassGroup[]): ClassGroup[] {
...
function updateBooksJson(classes: ClassGroup[]): void {
  if (!CONFIG.updateJson) return;
  try {
...
async function downloadAllBooks() {
  try {
    ...
    let classes = JSON.parse(booksData) as ClassGroup[];
    ...
    for (const cls of classes) {
      for (const subject of cls.subjects) {
        for (const book of subject.books) {
          if (book.coverUrl) stats.totalFiles++;
